...
class CustomItemService {
    ...
    postDBLoad() {
        ...
        for (const itemId in this.configs) {
            ...
            this.processStaticQuestItems(itemConfig, itemId);
            //this.processStaticLooseItems(itemConfig, itemId);
            this.processStaticLootLocations(itemConfig, itemId);
        ...
        if (numItemsAdded > 0) {
            this.Instance.logger.log(`[${this.Instance.modName}] Database: Loaded ${numItemsAdded} custom items.`, LogTextColor_1.LogTextColor.MAGENTA);
        }
        else {
            this.Instance.logger.log(`[${this.Instance.modName}] Database: No custom items loaded.`, LogTextColor_1.LogTextColor.MAGENTA);
        }
        for (const itemId in this.configs) {
            const itemConfig = this.configs[itemId];
   ...
   */
    ...
    addToStaticLootLocation(itemId, probability) {
        //const locations = this.Instance.database.locations;
        //if (locations) {
        //for (const locationID in locations) {
        //if (locations.hasOwnProperty(locationID)) {
        ...
        //if (location) {
        //const staticLoot: ItemDistribution = {
        //tpl: itemId,
        //relativeProbability: probability
        //};
        //}
    ...
    processStaticLootLocations(itemConfig, itemId) {
        if (itemConfig.addLooseLoot) {
            if (this.Instance.debug) {
                console.log("Processing Static Loot Locations for: ", itemId);
            }
            this.addToStaticLootLocation(itemId, itemConfig.addLooseLootProbability);
        }
    }
    /**
     *
     * @param mapLocation - The map location to add the item spawnpoint to.
     * @param locationID  - The ID of the location to add the item spawnpoint to.
     * @param Id - The ID of the spawnpoint.
     * @param randomRotation - Whether the item should have a random rotation.
     * @param position - The position of the item.
     * @param rotation - The rotation of the item.
     * @param root - The root of the item.
     * @param _id - The ID of the item.
     * @param _tpl - The template of the item.
     */
    addToStaticLooseLocation(locationID, Id, randomRotation, position, rotation, root, _id, _tpl) {
        const locations = this.Instance.database.locations;
        for (const locationIDs in locations) {
            if (locations.hasOwnProperty(locationIDs)) {
                const location = locations[locationIDs];
                if (location) {
                    const looseLocation = {
                    ...
                    if (location.looseLoot.spawnpointsForced) {
                        location.looseLoot.spawnpointsForced.push(looseLocation);
                    }
                    if (this.Instance.debug) {
                        console.log(`Added item to static loose location '${locationID}'`);
                    }
                }
            }
        }
     ...
     */
    processStaticLooseItems(itemConfig, itemId) {
        if (itemConfig.addLooseItemLocations) {
            if (this.Instance.debug) {
                console.log("Processing Loose Item to Static Loot Location for: ", itemId, " with LooseMapLocation: ", itemConfig.LooseMapLocations);
            }
            if (Array.isArray(itemConfig.StaticLooseLootLocations)) {
                if (this.Instance.debug) {
                    console.log("Processing Loose Item to Multiple Static Loot Locations:");
                }
                itemConfig.StaticLooseLootLocations.forEach((container) => {
                    const staticLooseLocations = container;
                    this.addToStaticLooseLocation(staticLooseLocations.locationId, staticLooseLocations.template.Id, staticLooseLocations.template.randomRotation, staticLooseLocations.template.Position, staticLooseLocations.template.Rotation, staticLooseLocations.template.Root, staticLooseLocations.template.Items[0]._id, staticLooseLocations.template.Items[0]._tpl);
                    if (this.Instance.debug) {
                        console.log(` - Added to location '${staticLooseLocations.locationId}'`);
                    }
                });
            }
        }
     ...
     * @param _tpl - The template of the item.
     */
    addToStaticQuestLocation(mapLocation, locationID, Id, randomRotation, position, rotation, root, _id, _tpl) {
     ...
     * @param itemId - The ID of the item.
     */
    processStaticQuestItems(itemConfig, itemId) {
        if (itemConfig.addQuestItemLocation) {
            ...
            if (Array.isArray(itemConfig.StaticQuestLootLocations)) {
                ...
                itemConfig.StaticQuestLootLocations.forEach((container) => {
                    ...
                    const staticQuestLocation = container;
                    this.addToStaticQuestLocation(mapLocation, staticQuestLocation.locationId, staticQuestLocation.template.Id, staticQuestLocation.template.randomRotation, staticQuestLocation.template.Position, staticQuestLocation.template.Rotation, staticQuestLocation.template.Root, staticQuestLocation.template.Items[0]._id, staticQuestLocation.template.Items[0]._tpl);
                    if (this.Instance.debug) {
   ...
   */
     ...
     * @param itemId
     */
    addtoGenerator(itemConfig, itemId) {
        const generator = this.Instance.database.hideout.areas.find((area) => area._id === "5d3b396e33c48f02b81cd9f3");
