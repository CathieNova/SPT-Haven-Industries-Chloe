...
export class CustomItemService {
   ...
   */
    ...
    private addToStaticLootLocation(itemId: string, probability: number): void {
        //const locations = this.Instance.database.locations;
    ...
    private processStaticLootLocations(itemConfig: any, itemId: string): void {
        if (itemConfig.addLooseLoot) {
            if (this.Instance.debug) {
                console.log("Processing Static Loot Locations for: ", itemId);
            }
            this.addToStaticLootLocation(
                itemId,
                itemConfig.addLooseLootProbability
            );
        }
    }
     ...
     */
    private addToStaticLooseLocation(locationID: string, Id: string, randomRotation: boolean, position: Ixyz, 
         ...
         */
    private processStaticLooseItems(itemConfig: any, itemId: string): void {
            ...
            if (Array.isArray(itemConfig.StaticLooseLootLocations)){
                ...
                itemConfig.StaticLooseLootLocations.forEach((container) => {
                    ...
                    this.addToStaticLooseLocation(
                        ...
                        staticLooseLocations.template.randomRotation,
                        staticLooseLocations.template.Position,
                        staticLooseLocations.template.Rotation,
                        staticLooseLocations.template.Root,
                        staticLooseLocations.template.Items[0]._id,
                        staticLooseLocations.template.Items[0]._tpl
                    );
                    if (this.Instance.debug) {
            ...
            if (Array.isArray(itemConfig.StaticQuestLootLocations)){
                ...
                itemConfig.StaticQuestLootLocations.forEach((container) => {
                    ...
                    this.addToStaticQuestLocation(
                        ...
                        staticQuestLocation.template.Id,
                        staticQuestLocation.template.randomRotation,
                        staticQuestLocation.template.Position,
                        staticQuestLocation.template.Rotation,
                        staticQuestLocation.template.Root,
